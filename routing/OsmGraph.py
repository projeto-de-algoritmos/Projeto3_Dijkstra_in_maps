...
def simple_adjlist(graph, node_coords):
    ...
    for vert in valid_nodes:
        ...
        for arc in valid_arcs:
            ...
            e_tuple = str(arc), haversine(node_ll[0], node_ll[1], node_coords[arc][0], node_coords[arc][1])
            adjlist[str(vert)].append(e_tuple)
    return dict(adjlist), needed_coords

if __name__ == '__main__':
    adjlist, node_coords = process_osm('map.osm')
    with open('brasilia.j', 'w') as fp:
        fp.write(json.dumps(adjlist))
    with open('brasilia_coords.j', 'w') as fp:
