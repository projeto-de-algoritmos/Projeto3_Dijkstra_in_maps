  ...
  def endElement(self, name):
    ...
    if name == 'way':
        ...
        if last != -1:
          for routeType in self.routeTypes:
            if(access[routeType]):
              ...
              weight = 1
              wlist = ['primary', 'secondary', 'tertiary']
              if self.tags.get('highway') in wlist:
                  weight = .9
              self.addLink(last, i, routeType, name, weight)#weight)
              if reversible or routeType == 'foot':
                self.addLink(i, last, routeType, name, weight)#weight)
        last = i
      ...
      if(self.storeMap):
        ...
        wayType = self.tags
#        if(wayType):
#          for node in self.waynodes:
#              if 'name' in wayType:
#                  try:
#                    self.namedways[node].append(wayType['name'])
#                  except:
#                    self.namedways[node] = [wayType['name']]
#              elif 'tiger:name_base' in wayType and 'tiger:name_type' in wayType:
#                  try:
#                    self.namedways[node].append(wayType['tiger:name_base'] + " " 
#                                                + wayType['tiger:name_type'])
#                  except KeyError:
#                    self.namedways[node] = [(wayType['tiger:name_base'] + " " 
#                                          + wayType['tiger:name_type'])]
  
  def addLink(self, fr, to, routeType,name, weight):
    """Add a routeable edge to the scenario"""
    self.routeablefrom(fr, routeType)
    try:
      if to in self.routing[routeType][fr].keys():
        return
      self.routing[routeType][fr][to] = {'weight' : weight,
                                          'name' : name, 'bearing': None}
    except KeyError:
      self.routing[routeType][fr] = {to: {'weight': weight, 'name' : name, 'bearing': None}}
  ...
  def find_node(self, lat, lon, routeType):
